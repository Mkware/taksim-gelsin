...
- **`smart_matching.service.ts`** is the active driver-selection algorithm. (The previous implementation, `matching.service.old.ts`, was deleted 15 Tem 2026 — Faz 3 madde 11; git history is the reference now if needed.) The matcher keeps per-ride state in Redis (`ride:matching:*`, `ride:rejected:*`, `ride:pending:*`, `driver:pending_offer:*`, `driver:penalty:*`, `driver:timeout_streak:*`) — debugging matching usually means inspecting these keys, not the database.
  - **Parallel offer wave** (added 15 Tem 2026): `platform_settings.matchingOfferWaveSize` (1–3, default 1 = sequential) keeps up to N offers outstanding at once; a reject/timeout refills the wave from the queue immediately. `ride:pending:{rideId}` is a Redis **SET** of driverIds (the Lua scripts auto-convert a legacy string value from an older deploy). Per-offer UI deadlines live at `ride:driver_response_deadline:{rideId}:{driverId}`. The true accept arbiter is unchanged — the atomic `accept_ride_with_fee` SQL function (`searching→accepted`, first wins); a concurrent loser gets `RIDE_UNAVAILABLE` with **no wallet deduction**, and on a win the rest of the wave receives `ride:request_cancelled` (`accepted_by_other`).
  - **Ranking is by ETA, not raw meters** (added 15 Tem 2026): `driving_distance.service.ts`'s `drivingMetersAndSecondsDriverToPickup()` returns both `{ meters, seconds }` from Google Distance Matrix (Redis cache value format `"meters:seconds"`, prefix bumped to `gdm:v2`); `scoreAndRankDrivers()` bands and scores on `duration_s` (`DURATION_BAND_S` = 90s, analogous to the old 400m `DISTANCE_BAND_M`) — a driver 400m away but 8 minutes out in traffic no longer beats one 600m away but 3 minutes out. `distance_m` is still carried on `NearbyDriver`/`ScoredDriver` for display/logging only.
  - **Second search wave** (added 15 Tem 2026): when the queue empties with zero outstanding offers, `_fillOfferWaveInner` calls `retrySearchOnce()` — a real `find_nearby_drivers` re-search (same `searchAndEnqueueDrivers()` helper `startSmartMatching` uses) — exactly once per ride (guarded by `ride:matching:retry_used:{rideId}`, `SET NX`) before giving up and cancelling. Catches drivers who came online or freed up mid-search; previously-rejected drivers for that ride are still excluded (rejected-set check happens inside the re-search too).
  - `scoreAndRankDrivers()` ranks candidates by `DISTANCE_BAND_M` (400m) bands first, then by weighted score (`WEIGHTS`: distance/dailyRides/rating/acceptanceRate) within the same band — fixed 15 Tem 2026, previously it sorted by raw metres with score only breaking ties under 1m apart (i.e. the score essentially never mattered in practice).
  - Eligibility filtering (`filterCandidatesForRide()` — online/available/balance/live-socket/offer-lock) now runs on the raw RPC candidate list *before* the paid Google Distance Matrix call, not after — previously ineligible candidates were still sent to Matrix and paid for.
- **`platform_settings.service.ts`** — operational knobs (`rideAcceptFeePercent`, `minDriverOnlineBalanceTcoin`, `pickupMaskRadiusM`, `driverResponseTimeoutSeconds`, `matchingOfferWaveSize`, `matchingSearchRadiusM`, `matchingMaxDriversPerRide`, `matchingTimeoutBanThreshold`, `matchingTimeoutBanSeconds`, `tariffBaseFare`/`tariffPerKmRate`/`tariffMinimumFare`/`tariffWaitingRatePerMinute`, etc.) live in the Supabase `platform_settings` table and are mutated at runtime by admins via `PUT /api/v1/admin/settings/platform` (admin-panel's Settings page). The corresponding env vars in `.env` are **only bootstrap defaults** for an empty DB — never treat env as the source of truth for these values. The taxi tariff moved here from a hardcoded constant in `modules/ride/pricing.service.ts` on 15 Tem 2026 (Faz 3, madde 12) — `pricing.service.ts` now just reads `getPlatformSettings()` on every call.
