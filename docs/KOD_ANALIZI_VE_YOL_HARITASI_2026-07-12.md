...
**Not — TS servis katmanı vs. SQL/Redis testleri:** Backend `supabaseAdmin`'i (supabase-js → PostgREST) ve `redis`'i modül seviyesinde singleton olarak import ediyor (DI yok, bkz. CLAUDE.md). `smart_matching.service.ts` yalnızca Redis'e dokunduğu için env değişkenlerini (bkz. `tests/support/env.ts`) sahte-ama-geçerli değerlerle doldurup gerçek modülü import etmek yeterli oldu. Ama `ride.service.ts`/`auth.service.ts` gibi Supabase'e (PostgREST) yazan servisleri gerçekten import edip çağırmak için ya tam bir yerel Supabase stack'i (Postgres+PostgREST, `supabase` CLI ile) ya da testcontainers ile ayrı bir `postgrest/postgrest` konteyneri gerekir — bu henüz kurulmadı. Şimdilik bu servislerin DB seviyesindeki garantileri (koşullu UPDATE, trigger, constraint) doğrudan SQL ile test ediliyor; TS katmanının kendisi (yetki kontrolü, hata mesajları, wallet refund çağrısı vb.) test kapsamı dışında kalıyor.
