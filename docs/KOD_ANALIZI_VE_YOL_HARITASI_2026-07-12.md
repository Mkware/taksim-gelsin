...
- [x] 7b. Ride durum geçişleri test edildi (14 Tem 2026, `tests/integration/ride_status_transitions.test.ts`) — tam yaşam döngüsü (accepted→arriving→in_progress→completed), koşullu UPDATE'in bayat/eşzamanlı geçişleri reddetmesi, `increment_driver_total_rides` trigger'ı, `uniq_customer_active_ride` partial unique index. SQL seviyesinde (ride.service.ts'nin kullandığı aynı "koşullu UPDATE" deseni doğrudan test ediliyor — TS servis katmanı supabase-js/PostgREST gerektirdiği için henüz kapsam dışı, bkz. not aşağıda). session_version akışı henüz test edilmedi (TS auth.service.ts katmanı, aynı PostgREST kısıtı geçerli).
- [x] 7c. Redis testcontainer kuruldu (14 Tem 2026, `tests/integration/smart_matching_queue.test.ts`) — `smart_matching.service.ts`'nin `ACQUIRE_NEXT_DRIVER_LUA` atomik claim script'i: çevrimdışı/kilitli sürücüleri atlama, kuyruk tükenince null dönme, eşzamanlı çağrılarda aynı sürücünün iki kez devredilmemesi. Test edilebilmesi için `acquireNextDriver`/`REDIS_KEYS`/`DRIVER_SOCKET_KEY`/`DRIVER_PENDING_OFFER_PREFIX`/`OFFER_DEADLINES_ZSET` modülde `export` edildi (davranış değişmedi, yalnızca görünürlük).
- [x] 8. Kritik Socket akışı test edildi (14 Tem 2026, `tests/integration/ride_matching_socket_flow.test.ts`) — gerçek Socket.io sunucusu + `socket.io-client` bağlantıları + gerçek Postgres (PostgREST üzerinden) + gerçek Redis'e karşı: sürücü `driver:go_online` ile bağlanıp odaya katılıyor, `sendRequestToNextDriver()` gerçek `ride:new_request` event'ini gönderiyor, birinci sürücü yanıt vermeyince gerçek zamanlayıcı (5sn, `DRIVER_RESPONSE_TIMEOUT_SECONDS`) dolup `handleOfferTimeout` → `acquireNextDriver` zinciriyle otomatik olarak ikinci sürücüye geçiyor. Kapsam dışı: `startSmartMatching()`'in "yakın sürücü bul + Google Distance Matrix ile sırala" aşaması (harici API bağımlılığı) — test doğrudan `sendRequestToNextDriver()`'dan başlıyor, aday kuyruğunu Redis'e elle yazıyor.

**Bu turda ikinci gerçek şema hatası bulundu:** `users.session_version` kolonu **hiçbir migration dosyasında tanımlı değildi** — ama `auth.service.ts`, `auth.middleware.ts`, `sockets/middleware/socket.auth.ts` bunu aktif okuyup yazıyor ve CLAUDE.md var olan bir kolon olarak belgeliyor. `009_user_session_version.sql` eklendi.

**Üretim doğrulaması (14 Tem 2026) — her iki bulgu da canlıda sorun değilmiş:**
- `session_version`: `backend/.env`'deki gerçek `SUPABASE_URL`/`SUPABASE_ANON_KEY` ile `GET /rest/v1/users?select=session_version&limit=0` → `200 []` (kolon var, PostgREST şema doğrulamasını geçti).
- `deduct_driver_balance`: `SELECT pg_get_function_result(oid) FROM pg_proc WHERE proname = 'deduct_driver_balance'` → `BOOLEAN` (kullanıcı SQL editöründen doğruladı).

Yani ikisi de daha önce SQL editöründen elle düzeltilmiş/eklenmiş ama migration dosyalarına hiç yansımamış — **tam olarak madde 9'un ("migration takibi kırık") somut kanıtı.**

**Not — TS servis katmanı vs. SQL/Redis testleri:** Backend `supabaseAdmin`'i (supabase-js → PostgREST) ve `redis`'i modül seviyesinde singleton olarak import ediyor (DI yok, bkz. CLAUDE.md). `smart_matching.service.ts` gibi yalnızca Redis'e dokunan servisler için env değişkenlerini (bkz. `tests/support/env.ts`) sahte-ama-geçerli değerlerle doldurup gerçek modülü import etmek yeterliydi. `ride.service.ts`/`auth.service.ts` gibi Supabase'e (PostgREST) yazan servisleri gerçekten import edip çağırmak için `tests/support/postgrest_stack.ts` ile testcontainers'ta gerçek bir `postgrest/postgrest` konteyneri (+ `authenticator`/`service_role` rol kurulumu + `/rest/v1` path'ini soyan minik bir Node proxy, çünkü PostgREST path prefix desteklemiyor ve gerçek Supabase'de bunu Kong hallediyor) kuruldu — artık madde 8'deki gibi servisler gerçek haliyle test edilebiliyor. `ride.service.ts`/`auth.service.ts`'in kendisi için ayrı testler henüz yazılmadı (yalnızca smart_matching.service.ts bu altyapıyı kullanıyor); bu altyapı sayesinde ileride yazılması ucuzlaştı.
